...
public class PhysicsDemo implements Updatable {
	...
	public void createRobot() {
		for (int i = 0; i < motorCommands.length; i++) {
			motorCommands[i] = new Random().nextFloat() * (float) Math.PI
					- (float) (Math.PI / 2);
		}
		List<Integer> attachmentLocs = Lists.newArrayList(0, 1, 0, 3, 0, 5, 0,
				7);
		List<Float> perimeterLocs = Lists.newArrayList(0.25f, 0.5f, 0.5f, 0.5f,
				0.75f, 0.5f, 1f, 0.5f);

		body = new RobotBody(physics, Lists.newArrayList(0, 1, 0, 3,
				0, 5, 0, 7), Lists.newArrayList(0.25f, 0.5f, 0.5f, 0.5f, 0.75f,
				0.5f, 1f, 0.5f));
		// body = new
		// RobotBody(physics,Lists.newArrayList(0,1,0,3),Lists.newArrayList(0.375f,0.5f,0.625f,0.5f));
	}
	...
	public void update(float deltaTime) {
		if (body != null) {
			for (int i = 0; i < body.getJoints().size(); i++) {
				body.getJoints().get(i).sendMotorCommand(motorCommands[i]);
				body.getJoints().get(i)
						.actuate(physics, deltaTime);

				System.out.println(body.getJoints().get(i).getCurrentAngle());
				
				motorCommands[i] += 0.5f * (float) (Math.random() - 0.5);
			}
		}
	}
